            ...
            if old_delay > 1:
                if iterative:
                    ...
                    for new_neuron in range(neurons_to_add):
                        ...
                        graph.nodes[to_add_neuron]['potential'] = 0.0
                        graph.nodes[to_add_neuron]['threshold'] = 0.5
                        graph.add_edge(previous_neuron, to_add_neuron, weight=1.0, delay=1)
                        previous_neuron = to_add_neuron
                    graph.add_edge(previous_neuron, to_neuron, weight=graph[from_neuron][to_neuron]['weight'], delay=1)
                    graph.remove_edge(edge[0], edge[1])
        max_delay = np.max([graph[edge[0]][edge[1]]['delay'] for edge in graph.edges()])
...
def get_neuron_tensors(graph, batch_size=1000, verbose=0, mode='cpu', sparse=True):
    neuron_tensors = dict()
    if mode == 'cpu':
        torch.set_default_tensor_type(torch.FloatTensor)
    elif mode == 'cuda':
        ...
        if sparse:
            ...
            for j, neuron in enumerate(neuron_group[2]):
                ...
                for in_edge in in_edges:
                    ...
                    weight_deque.append(graph[in_neighbor][this_node]['weight'])
            weights = torch.sparse_coo_tensor([list(row_indices), list(column_indices)], list(weight_deque), (len(graph.nodes()), len(neuron_group[2])) )
        else:
...
def step_simulation(neuron_tensors, spikes = None, injection=None, watches=[], verbose=0):
    ...
    for i, ten_num in enumerate(neuron_tensors):
        ...
        if spikes is not None:
            spikes = spikes.reshape((spikes.numel(),1))  #Needed for sparse matrix
            integrated_spikes = torch.matmul(n_tensor['weight'].t(), spikes)
            new_potential = n_tensor['potential'] + integrated_spikes.reshape((integrated_spikes.numel(),))
...
def run_simulation(graph, timesteps, injection_dictionary, start_time=0,batch_size=1000,watches=[], verbose = 0, mode='cpu',sparse=True):
    neuron_tensors = get_neuron_tensors(graph, batch_size=batch_size, mode=mode, sparse=sparse)
