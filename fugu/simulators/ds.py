...
def remove_delay(graph, verbose=0, iterative=False):
    max_delay = np.max(
        [graph[edge[0]][edge[1]]['delay'] for edge in graph.edges()])
    iteration = 0
    while max_delay > 1:
        ...
        for i, edge in enumerate(list(graph.edges())):
            ...
            if old_delay > 1:
                if iterative:
                    ...
                    neurons_to_add = old_delay - 1
                    previous_neuron = from_neuron
                    for new_neuron in range(neurons_to_add):
                        to_add_neuron = graph.number_of_nodes()
                        graph.add_node(to_add_neuron)
                        graph.nodes[to_add_neuron]['potential'] = 0.0
                        graph.nodes[to_add_neuron]['threshold'] = 0.5
                        graph.add_edge(previous_neuron,
                                       to_add_neuron,
                                       weight=1.0,
                                       delay=1)
                        previous_neuron = to_add_neuron
                    graph.add_edge(
                        previous_neuron,
                        to_neuron,
                        weight=graph[from_neuron][to_neuron]['weight'],
...
def restore_delay(graph):
    ...
    while not done:
        ...
        for edge in edges_list:
            if graph.out_degree(edge[1]) == 1 and graph[edge[0]][
                    edge[1]]['weight'] > graph.node[edge[1]]['threshold']:
...
def initialize_worker_blocks(graph,
                             mode='cpu',
                             batch_size=1000,
                             num_workers=1):
    ...
    for neuron_type in neuron_type_dict.keys():
        ...
        for start_idx in range(0, len(neurons), batch_size):
            ...
            job_blocks.append(
                (len(job_blocks), neuron_type, neurons[start_idx:stop_idx]))
...
def separate_types(graph, batch_size=1000):
    ...
    for neuron in neuron_list:
        types = [_IF_TYPE]
        if 'decay' in graph.nodes[
                neuron] and graph.nodes[neuron]['decay'] < 1.0:
            types.append(_LEAKY_TYPE)
        if 'p' in graph.nodes[neuron] and graph.nodes[neuron]['p'] < 1.0:
            types.append(_STOCHASTIC_TYPE)
...
def initialize_neuron_states(graph, neuron_list, neuron_type, mode='cpu'):
    ...
    if check_type(neuron_type, _IF_TYPE):
        threshold = torch.tensor([
            graph.nodes[neuron]['threshold'] for neuron in neuron_list
        ]).float()
        potential = torch.tensor([
            graph.nodes[neuron]['potential'] for neuron in neuron_list
        ]).float()
        states['threshold'] = threshold
        states['potential'] = potential
    if check_type(neuron_type, _LEAKY_TYPE):
        decay = torch.tensor(
            [graph.nodes[neuron]['decay'] for neuron in neuron_list]).float()
        states['decay'] = decay
    if check_type(neuron_type, _STOCHASTIC_TYPE):
        p = torch.tensor([graph.nodes[neuron]['p']
                          for neuron in neuron_list]).float()
        states['p'] = p
    return states


def get_neuron_tensors(graph,
                       batch_size=1000,
                       verbose=0,
                       mode='cpu',
                       sparse=True):
    ...
    for i, neuron_group in enumerate(
            separate_types(graph, batch_size=batch_size)):
        neuron_tensors[i] = initialize_neuron_states(graph,
                                                     neuron_group[2],
                                                     neuron_group[1],
                                                     mode=mode)
        if mode == 'cpu':
            neuron_tensors[i]['index'] = torch.tensor(neuron_group[2],
                                                      dtype=torch.long)
        elif mode == 'cuda':
            neuron_tensors[i]['index'] = torch.tensor(neuron_group[2],
                                                      dtype=torch.long)
        if sparse:
            ...
            for j, neuron in enumerate(neuron_group[2]):
                ...
                for in_edge in in_edges:
                    ...
                    this_node = in_edge[1]
                    row_indices.append(in_neighbor)
                    column_indices.append(j)
                    weight_deque.append(
                        graph[in_neighbor][this_node]['weight'])
            weights = torch.sparse_coo_tensor(
                [list(row_indices), list(column_indices)], list(weight_deque),
                (len(graph.nodes()), len(neuron_group[2])))
        else:
            ...
            for j, neuron in enumerate(neuron_group[2]):
                ...
                for in_edge in in_edges:
                    ...
                    this_node = in_edge[1]
                    weights[in_neighbor,
                            j] = graph[in_neighbor][this_node]['weight']
        neuron_tensors[i]['weight'] = weights
    return neuron_tensors


def step_simulation(neuron_tensors,
                    spikes=None,
                    injection=None,
                    watches=[],
                    verbose=0):
    number_of_neurons = np.sum([
        neuron_tensors[ten_num]['index'].numel() for ten_num in neuron_tensors
    ])
    ...
    for i, ten_num in enumerate(neuron_tensors):
        ...
        if injection is not None:
            ...
            n_tensor['potential'] = n_tensor['potential'] + injection[
                n_tensor['index']]
        #Integrate Spikes
        if spikes is not None:
            spikes = spikes.reshape(
                (spikes.numel(), 1))  #Needed for sparse matrix
            integrated_spikes = torch.matmul(n_tensor['weight'].t(), spikes)
            new_potential = n_tensor['potential'] + integrated_spikes.reshape(
                (integrated_spikes.numel(), ))
            n_tensor['potential'] = new_potential
        if check_type(neuron_type, _RECORD_PREACTIVATION):
            if 'preactivation_history' not in n_tensor:
                if verbose > 0:
                    print("Creating Preactivation History for tensor " +
                          str(ten_num) + '.')
                    n_tensor['preactivation_history'] = deque()
            n_tensor['preactivation_history'].append(
                (len(n_tensor['preactivation_history']),
        ...
        #Decay
        if check_type(neuron_type, _LEAKY_TYPE):
            n_tensor['potential'] = n_tensor['potential'] * (1 -
                                                             n_tensor['decay'])
        ...
        if check_type(neuron_type, _RECORD_SPIKES):
            ...
            n_tensor['spike_history'].append(
                (len(n_tensor['spike_history']),
        ...
        if check_type(neuron_type, _RECORD_POTENTIAL):
            ...
            n_tensor['potential_history'].append(
                (len(n_tensor['potential_history']),
                 n_tensor['potential'].clone()))
...
def run_simulation(graph,
                   ...
                   mode='cpu',
                   sparse=True):
    neuron_tensors = get_neuron_tensors(graph,
    ...
    for t in range(start_time, start_time + timesteps):
        if verbose > 0:
            print("{0:.2f}".format(100 * t / timesteps) + '% complete.',
                  end='\r')
        if t in injection_dictionary:
            ...
            injection = None
        neuron_tensors, spikes = step_simulation(neuron_tensors,
                                                 spikes,
                                                 injection,
                                                 watches,
                                                 verbose=verbose)
...
def get_injection_tensor_from_initial_walkers(initial_dictionary,
                                              graph,
                                              num_neurons,
                                              mode='cpu'):
    if mode == 'cpu':
