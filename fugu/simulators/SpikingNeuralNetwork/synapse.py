...
class LearningSynapse(Synapse):
    ...
    Parameters:
        ...
        learning_rule (str): optional.  The learning rule to be applied to the synapse. Currently provides options among (["STDP", "three-factor"]). The default is None.
        Delay (int) : non-negative Int, optional.  Number of time steps needed to relay the scaled spike. The default is 1.
        Weight (double): optional.  Scaling value for incoming spike. The default is 1.0.
        Mod_neuron (Neuron): optional. Modulatory neuron for transmitting either errors or other modulatory signals
    """

    def __init__(self, pre_neuron: Neuron, post_neuron: Neuron, learning_rule: str = "STDP", delay: int = 1, weight: float = 1.0, mod_neuron: Neuron = None, learning_params=None):
        if not isinstance(pre_neuron, Neuron) or not isinstance(post_neuron, Neuron):
        ...
        if learning_rule not in ["STDP", "r-STDP", "three-factor"]:
            raise ValueError("Learning rule must be one of the following: ['STDP', 'r-STDP', 'three-factor']")
    ...
    def __str__(self):
        return "{0}_Synapse {1}({2}, {3})".format(self._name_learning_rule, self.name, self._d, self._w)
    ...
    def show_params(self):
        ...
        print("Synapse {0} -> {1}:\n delay  : {2}\n weight : {3}\n learning_rule : {4}".format(self._pre, self._post, self._d, self._w, self._name_learning_rule))
        ...
        if self._learning_rule == "STDP":
            if self._post.spike:
                ...
                if pre_spike_hist[delay_shift : delay_shift + 1][0]:
                    ...
                    if pre_spike_time != 0:
                        self._w += self._learning_params.A_p * np.exp((-pre_spike_time) / self._learning_params.tau)
                    else:
                        self._w += 0
            ...
            # If the post synaptic neuron does not spike
                ...
                if pre_spike_hist[delay_shift : delay_shift + 1][0]:
                    ...
                    if post_spike_time != 0:
                        self._w += self._learning_params.A_n * np.exp((-post_spike_time) / self._learning_params.tau)
                    else:
                        self._w += 0
                else:
                    self._w += 0
