...
class ds_Backend(Backend):
    ...
    def _create_ds_injection(self, input_values):
        ...
        for t in input_values:
            injection_tensors[t] = torch.zeros(
                (self.scaffold.graph.number_of_nodes(), )).float()
            spiking_neurons = [
                self.scaffold.graph.nodes[neuron]['neuron_number']
                for neuron in input_values[t]
            ]
    ...
    def compile(self, scaffold, compile_args={}):
        ...
        record = compile_args[
            'record'] if 'record' in compile_args else 'output'
            ...
            for node in self.scaffold.circuit.nodes:
                if 'layer' in self.scaffold.circuit.nodes[node]:
                    if self.scaffold.circuit.nodes[node]['layer'] == 'output':
                        for o_list in self.scaffold.circuit.nodes[node][
                                'output_lists']:
                            for neuron in o_list:
                                self.scaffold.graph.nodes[neuron]['record'] = [
                                    'spikes'
                                ]
        self.ds_graph = nx.convert_node_labels_to_integers(
            self.scaffold.graph, label_attribute='name')
    ...
    def reset(self):
        ...
        for node in self.scaffold.graph.nodes:
            ...
            if 'potential' in properties:
                self.ds_graph.nodes[neuron_id]['potential'] = properties[
                    'potential']
            else:
    ...
    def set_properties(self, properties={}):
        ...
        for brick in properties:
            if brick != 'compile_args':
                ...
                changes = self.scaffold.circuit.nodes[brick_id][
                ...
                if changes:
                    ...
                    for neuron in neuron_props:
                        ...
                        for prop in properties:
                            self.ds_graph.nodes[neuron_id][prop] = properties[
                                prop]

                    for synapse in synapse_props:
                        if type(synapse) is tuple:
                            pre, post = [
                                self.neuron_to_id_map[p] for p in synapse
                            ]
                            properties = synapse_props[synapse]
                            for prop in properties:
                                self.ds_graph.edges[
                                    pre, post][prop] = properties[prop]
                        else:
