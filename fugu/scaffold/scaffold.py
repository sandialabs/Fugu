...
class Scaffold:
    ...
    def add_brick(self,
        ...
        node_tags = {
            self.circuit.nodes[node]['tag']: node
        ...
        for node in input_nodes:
            ...
            if type(processed_node) is tuple and type(
                    processed_node[0]) is str:
                processed_node = (node_tags[processed_node[0]],
                                  processed_node[1])
    ...
    def resolve_timing(self):
        ...
        for edge in edges:
            self.circuit.edges[edge[0],
                               edge[1]]['weight'] = self.circuit.nodes[
                                   edge[0]]['brick'].metadata['D']
        ...
        for node in reversed(nodes):
            ...
            print(node, list(pred_nodes), len(list(pred_nodes)),
                  len(pred_nodes))
            ...
            if len(list(pred_nodes)) < 2:
                ...
                for target_node in list(pred_nodes):
                    distance_guess = 0
                    target_paths = nx.all_simple_paths(self.circuit, 0,
                                                       target_node)
                    ...
                    distance_guess = distance_guess + self.circuit.edges[
                        target_node, node]['weight']
                ...
                for i in range(0, len(pred_nodes)):
                    ...
                    if (distances[i] < max_value):
                        ...
                        print(
                            'Adding delay node of length {} between {} and {}'.
                            format(
                        ...
                        self.pos[target_node + 0.5] = np.array(
                            [target_node + 0.5, self.pos[target_node][1]])
    ...
    def all_nodes_built(self, verbose=0):
        ...
        if verbose > 0:
            ...
            for node in self.circuit.nodes:
                print("{}:{}".format(
                    node, self.circuit.nodes[node]['brick'].is_built))
        return b
    ...
    def lay_bricks(self, verbose=0):
        ...
        for node in [
                node for node in self.circuit.nodes
                if 'layer' in self.circuit.nodes[node]
                and self.circuit.nodes[node]['layer'] == 'input'
        ]:
            (built_graph, metadata, control_nodes, output_lists,
             output_codings) = self.circuit.nodes[node]['brick'].build(
                 built_graph, None, None, None, None)
            self._assign_brick_tags(built_graph,
                                    self.circuit.nodes[node]['tag'])
        ...
        while not self.all_nodes_built(verbose=verbose):
            ...
            for node in [
                ...
                for input_number in range(
                        0, len(self.circuit.nodes[node]['input_nodes'])):
                    if verbose > 0:
                        print("Processing input: {}".format(input_number))
                    inputs[input_number] = {
                        'input_node':
                        self.circuit.nodes[node]['input_nodes'][input_number]
                        [0],
                        'input_channel':
                        self.circuit.nodes[node]['input_nodes'][input_number]
                        [1],
                ...
                input_codings = []
                for key in inputs:
                    circuit_node = self.circuit.nodes[inputs[key]
                                                      ['input_node']]
                    input_channel = inputs[key]['input_channel']
                    metadata.append(circuit_node['metadata'])
                    control_nodes.append(
                        circuit_node['control_nodes'][input_channel])
                    input_lists.append(
                        circuit_node['output_lists'][input_channel])
                    input_codings.append(
                        circuit_node['output_codings'][input_channel])
