...
class Simple(Brick):
    ...
    def build(self, graph, dimensionality, control_nodes, input_lists,
              input_codings):
            ...
            for i in range(idx_num - self.ctr, idx_num + self.ctr + 1):
                if (i >= 0 and i < self.max_cols):
                    graph.add_edge(input_lists[0][i],
                                   ...
                                   delay=1.0)
            if self.surr > 0:
                for i in range(idx_num - self.ctr - self.surr,
                               idx_num - self.ctr):
                    if (i >= 0 and i < self.max_cols):
                        graph.add_edge(input_lists[0][i],
                                       simple_node_name,
                                       ...
                                       delay=1.0)
                for i in range(idx_num + self.ctr + 1,
                               idx_num + self.ctr + self.surr + 1):
                    if (i >= 0 and i < self.max_cols):
                        graph.add_edge(input_lists[0][i],
                                       simple_node_name,
                                       weight=-self.surr_weight,
        ...
        return (graph, self.dimensionality, control_nodes, output_lists,
                output_codings)
    ...
    def build(self, graph, dimensionality, control_nodes, input_lists,
        ...
        for idx_num, operand0 in enumerate(input_lists[0]):
            ...
            graph.add_node(ds_node_name,
                           ...
                           p=1.0,
                           potential=0.0)
            graph.add_edge(operand0, ds_node_name, weight=0.5, delay=1.0)
            if (idx_num - self.dir_step >= 0
                    and idx_num - self.dir_step < self.max_cols):
    ...
    def build(self, graph, dimensionality, control_nodes, input_lists,
        ...
        for idx_num, operand0 in enumerate(input_lists[0]):
            ...
            graph.add_node(pre1_node_name,
                           ...
                           p=1.0,
                           potential=0.0)
            graph.add_edge(operand0, pre1_node_name, weight=0.5, delay=1.0)
            if (idx_num - self.dir_step >= 0
                    and idx_num - self.dir_step < self.max_cols):
                graph.add_edge(input_lists[0][idx_num - self.dir_step],
                               pre1_node_name,
        ...
        for idx_num, operand0 in enumerate(input_lists[0]):
            ...
            graph.add_node(pre2_node_name,
                           ...
                           p=1.0,
                           potential=0.0)
            graph.add_edge(operand0, pre2_node_name, weight=0.5, delay=1.0)
            if (idx_num + self.dir_step >= 0
                    and idx_num + self.dir_step < self.max_cols):
                graph.add_edge(input_lists[0][idx_num + self.dir_step],
                               pre2_node_name,
        ...
        for idx_num, operand0 in enumerate(input_lists[0]):
            ...
            if (idx_num - self.dir_step >= 0
                    and idx_num - self.dir_step < self.max_cols):
                pre2_node_name = 'half+_' + self.name + '_' + input_lists[0][
                    idx_num -
                    self.dir_step]  ### MY PROBLEM IS ADDING THIS EDGE...
...
numcols = 10
numframes = 50
Spiking_Input = np.zeros(
    (numcols,
     numframes))  ##shoud number of frames always be the first dimension?
stim_index = 0
...
scaffold = Scaffold()
scaffold.add_brick(
    Spike_Input(Spiking_Input,
                time_dimension='True',
                coding='Raster',
                name='InputSequence'), 'input')
scaffold.add_brick(hR_DS(name='hRds1', dir_step=1, max_cols=numcols), (0, 0),
                   output=True)
scaffold.add_brick(hR_DS(name='hRds2', dir_step=-1, max_cols=numcols), (0, 0),
                   output=True)
scaffold.add_brick(fR_DS(name='fRds1', dir_step=1, max_cols=numcols), (0, 0),
                   output=True)
scaffold.add_brick(fR_DS(name='fRds2', dir_step=-1, max_cols=numcols), (0, 0),
                   output=True)
scaffold.add_brick(Simple(name='simple1',
                          ctr=0,
                          ctr_weight=1.0,
                          surr=1,
                          surr_weight=0.5,
                          max_cols=numcols), (0, 0),
                   output=True)
scaffold.add_brick(Simple(name='simple2',
                          ctr=0,
                          ctr_weight=1.0,
                          surr=1,
                          surr_weight=0.4,
                          max_cols=numcols), (0, 0),
                   output=True)
scaffold.add_brick(hR_DS(name='hRds3', dir_step=1, max_cols=numcols), (5, 0),
                   output=True)
...
print(
    'binary input image sequence (transposed to each row is a time step)...: ')
print('\n\n-------------------------------------------------------\nInputs:')
