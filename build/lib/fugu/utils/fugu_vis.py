...
def graph_vis(result, scaffold, pos=None, debug=False, raster=True, check_spikes=None):
    """
    ...
    stylesheet_list.append({'selector': '.spikes',
                            'style': {
                                    ...
                                    'line-color': '#7D0D7C'}})
    stylesheet_list.append({'selector': '.nonspikes',
                            'style': {
    ...
    for node in scaffold.circuit.nodes():
        ...
        if 'layer' in scaffold.circuit.nodes[node]:    
            ...
            if scaffold.circuit.nodes[node]['layer'] == 'input':
                node_trace['marker']['color'] += tuple(['#A92C00']) #input nodes #DE5F76 
            else:
                node_trace['marker']['color'] += tuple(['#6CB312']) #output nodes #4B8F8C
        else:
            node_info = node_info + brick
            node_trace['marker']['color'] += tuple(['#FFA033']) #other nodes #2C365E
                                ...
                                html.Div(className="twelve columns", 
                                         children = [html.Div(className="footer",
                                                              children = [html.Div([html.Img(src='data:image/png;base64,{}'.format(doe.decode()), 
                                                                                             style={'width': '45%', 'margin-right':'0.5em', 'display': 'inline'}),
                                                                                    html.Img(src='data:image/png;base64,{}'.format(nnsa.decode()), 
                                                                                             style={'width':'45%', 'margin-right':'0.02em', 'display':'inline'})],
...
def _build_left_div(fig, elements_ls, stylesheet_list, time_step, max_t):
    return html.Div(className="six columns", 
                                       children = [dcc.Graph(id = 'scaffold_graph', 
                                                             figure = fig),
                                                   html.Div(className="twelve columns", 
                                                            children=[html.Label('scaffold.graph', 
                                                   ...
                                                   dcc.Textarea(id = 'text-area',
                                                                ...
                                                                style={'width': '100%',
                                                                       'background': '#FFFFFF'}),
                                                   html.P(children=[html.Div(className="twelve", 
                                                                             children=[html.Label("timestep", 
                                                                                                  style={'font-family': 'sans-serif', 
                                                                                                         'font-size': '2.0rem', 
                                                                                                         'background': '#FFFFFF',
                                                                                                         'margin-left': '0rem',
...
#        
    ...
    check_spikes = {0:{0:1, 1:0},
                    ...
                    13:{0:0, 1:0, 2:1},
                    14:{0:0, 1:0, 2:1}}
    graph_vis(result, scaffold, pos=pos, debug=False, check_spikes=check_spikes)
