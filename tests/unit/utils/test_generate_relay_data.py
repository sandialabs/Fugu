...
class TestGenerateRelayData:
    ...
    def test_no_relay(self, num_edges):
        ...
        assert relay_data.get_total_relays() == 0
        for edge in edges:
    ...
    def test_single_relay(self, max_delay):
        ...
        assert relay_data.get_total_relays() == 1
    ...
    def test_multiple_relay_chains(self):
        ...
        assert relay_data.get_total_relays() == sum(expected_total_relays)
        for edge, total_relays, final_delay in zip(edges, expected_total_relays, expected_final_delays):
            assert len(relay_data.get_relay_list(edge)) == total_relays
